...
import { Settings, Package, MapPin, Briefcase, TrendingUp, IndianRupee, Activity, Loader2 } from "lucide-react";
...
export default function PricingEngine() {
  ...
  const fetchEngine = useCallback(async () => {
    ...
    try {
      ...
      if (data.categoryPricing && Array.isArray(data.categoryPricing)) setCategoryPricing(data.categoryPricing);
  ...
  const updateCategoryPricing = (category: CategoryPricing['category'], field: keyof CategoryPricing, value: number | string) => {
  ...
  const handleSaveAll = async () => {
    ...
    for (const cp of categoryPricing) {
      const base = Number(cp.basePrice);
      const min = Number(cp.minPrice);
      const max = Number(cp.maxPrice);
      const margin = Number(cp.margin);
      const tax = Number(cp.tax);

      if (isNaN(base) || base <= 0) { showToast(`Invalid base price for ${cp.category}.`, 'error'); return; }
      if (isNaN(min) || min <= 0) { showToast(`Invalid min price for ${cp.category}.`, 'error'); return; }
      if (isNaN(max) || max <= 0) { showToast(`Invalid max price for ${cp.category}.`, 'error'); return; }
      if (min > base) { showToast(`Min price cannot be greater than base price for ${cp.category}.`, 'error'); return; }
      if (base > max) { showToast(`Base price cannot be greater than max price for ${cp.category}.`, 'error'); return; }
      if (isNaN(margin) || margin < 0 || margin > 100) { showToast(`Invalid margin for ${cp.category}. Must be 0-100.`, 'error'); return; }
      if (isNaN(tax) || tax < 0 || tax > 100) { showToast(`Invalid tax rate for ${cp.category}. Must be 0-100.`, 'error'); return; }
    }
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        ...
        <button onClick={fetchEngine} className="px-4 py-2 border border-gray-300 rounded-lg hover:bg-gray-50 text-sm font-medium text-gray-700 self-start">
          Refresh
        </button>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
        {[
          { icon: IndianRupee, color: 'text-blue-600', bg: 'bg-blue-50', value: '₹86.4K', label: 'Total Discount Impact' },
      ...
      <div className="bg-white rounded-lg border border-gray-200 p-6">
              ...
              return (
                <div key={cp.category} className={`border ${colors.border} rounded-lg p-6 ${colors.bg}`}>
                  ...
                  <div className="space-y-4">
                    <FormInput label="Base Price" type="number" value={cp.basePrice} 
                      onChange={v => updateCategoryPricing(cp.category, 'basePrice', v)} 
                      icon={<IndianRupee className="w-4 h-4" />} />
                    <div className="grid grid-cols-2 gap-3">
                      <FormInput label="Min Price" type="number" value={cp.minPrice} 
                        onChange={v => updateCategoryPricing(cp.category, 'minPrice', v)} 
                        icon={<IndianRupee className="w-3 h-3 text-xs" />} />
                      <FormInput label="Max Price" type="number" value={cp.maxPrice} 
                        onChange={v => updateCategoryPricing(cp.category, 'maxPrice', v)} 
                        icon={<IndianRupee className="w-3 h-3 text-xs" />} />
                    </div>
                    <div className="grid grid-cols-2 gap-3">
                      <div className="relative">
                        <FormInput label="Profit Margin" type="number" value={cp.margin} 
                          onChange={v => updateCategoryPricing(cp.category, 'margin', v)} />
                        <span className="absolute right-3 top-[38px] text-gray-500 text-xs">%</span>
                      </div>
                      <div className="relative">
                        <FormInput label="Tax Rate" type="number" value={cp.tax} 
                          onChange={v => updateCategoryPricing(cp.category, 'tax', v)} />
                        <span className="absolute right-3 top-[38px] text-gray-500 text-xs">%</span>
                      </div>
                    </div>
                  </div>
        ...
        <div className="flex gap-3 mt-6">
          <AdminButton onClick={handleSaveAll} isLoading={isSaving} icon={<Settings className="w-4 h-4" />}>
            Save All Changes
          </AdminButton>
              ...
              {discounts.length === 0 ? (
                ...
                return (
                  <tr key={discount.id} className="hover:bg-gray-50 transition-colors">
                    ...
                    <td className="px-4 py-4 whitespace-nowrap">
                      <AdminBadge variant={discount.status === 'active' ? 'green' : discount.status === 'scheduled' ? 'blue' : 'gray'}>
      ...
      <div className="bg-gradient-to-r from-blue-50 to-purple-50 border border-blue-200 rounded-lg p-6">
        ...
        <div className="mt-4">
          <AdminButton onClick={handleSaveAll} isLoading={isSaving} size="sm">
            Apply Global Rules
          </AdminButton>
        </div>
      </div>
